...
class AddAFriendTest < ActionDispatch::IntegrationTest
 ...
 test "that ading a friend works" do 
  sign_in_as users(:cyprian), "testing"

  get "/user_friendships/new?friend_id=#{users(:cyprian).profile_name}"
  assert_response :success 

  assert_difference 'UserFriendship.count' do
    post "/user_friendships", user_friendship: { friend_id: users(:blaize).profile_name }
